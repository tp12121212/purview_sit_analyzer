...
for i, line in enumerate(lines, start=1):
    ...
    # Split line into words without stripping characters
    raw_words = line.split()

    # Filter words: exclude stopwords, enforce min length, and reject words with non-alphanumeric chars
    valid_words = []
    for w in raw_words:
        ...
        if len(w) < min_keyword_length:
            continue
        if re.search(r'[^a-zA-Z0-9]', w):  # If any non-alphanumeric char exists, reject
            continue
        valid_words.append(w)

    # Build phrases only from adjacent valid words
    for start in range(len(valid_words)):
        for length in range(min_phrase_words, max_phrase_words + 1):
            if start + length <= len(valid_words):
                phrase = " ".join(valid_words[start:start+length])
        ...
        # ---- Keyword Extraction (Updated with Line Number) ----
    
for i, line in enumerate(lines, start=1):
    if not line.strip():
        continue

    # Split by spaces only (preserve @ and other symbols inside words)
    words_in_line = [w for w in line.split(" ")
                     if w and w.lower() not in stop_words and len(w) >= min_keyword_length]

    # Build phrases only from space-separated words
    for start in range(len(words_in_line)):
        for length in range(min_phrase_words, max_phrase_words + 1):
            if start + length <= len(words_in_line):
                phrase = " ".join(words_in_line[start:start+length])
                keyword_results.append({
                    'File_Name': file,
                    'Keyword': phrase,
                    'Line_Number': i,
